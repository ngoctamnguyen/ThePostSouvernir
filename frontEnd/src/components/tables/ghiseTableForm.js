...
function GhiseTableForm() {
  ...
  const tableRows = itemData.map((info, i) => {
    return (
      <tr key={i} style={{ backgroundColor: info.id % 2 === 0 ? "aqua" : "" }}>
        ...
        <td className='tableCenter'><Button variant="text" onClick={() => handleDelete(info.id)}>Delete</Button></td>
      </tr>
    );
  });
  ...
  return (
    <div>
      ...
      <GhiseDataForm func={addRows} checkout={checkout} tygia={tygia} quantity={quantity} grandTotal={grandTotal}/>
